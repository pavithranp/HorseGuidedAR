...
public class ARUWPController : MonoBehaviour {
    ...
    public Text mytext;
    public static bool ToggleTap = false;
    /// <summary>
    ...
    private void Update() {
        ...
        if(firstloop == 1 && status == ARUWP.ARUWP_STATUS_RUNNING)
        {
            Pause();
            firstloop = 0;
        }
        ...
        //Start Track on button click
        if(ViewModel.startTrack == true && status == ARUWP.ARUWP_STATUS_CTRL_INITIALIZED)
        {
            Resume();
            ViewModel.startTrack = false;
        }
        ...
        if (ToggleTap) {
            ...
            mytext.text = "Tap again to release.";
                Pause();
            }
            else if (status == ARUWP.ARUWP_STATUS_CTRL_INITIALIZED) {
            mytext.text = "Tap on the Cylinder to Lock.";
                Resume();
            }
        }
